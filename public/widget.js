(function () {
  ...
  const renderMarkers = () => {
    ...
    state.comments.forEach((comment) => {
      ...
      marker.className = `pl-marker ${getMarkerStatusClassName(comment.status)} ${
        state.selectedComment?.id === comment.id ? "pl-marker-selected" : ""
      }`;
  ...
  const renderPanel = () => {
    ...
    const visibleComments = state.comments.filter(
      (comment) => state.showResolved || comment.status !== "RESOLVED"
    );
    ...
    panel.innerHTML = `
      ...
      <button class="pl-panel-button" type="button" data-action="add-comment">
        + Add Comment
      </button>
              ...
              ${
                visibleComments.length
                  ? visibleComments
                      ...
                      .join("")
                  : `<div class="pl-comment-meta">No visible comments.</div>`
              }
            </div>
          `
          : ""
      ...
      <button class="pl-panel-button" type="button" data-action="close">
        Done Reviewing
      </button>
    `;
    ...
    panel.querySelectorAll(".pl-comments-list-item").forEach((button) => {
    ...
    document.addEventListener(
      ...
      (event) => {
        ...
        if (
          ...
          target.closest(".pl-comment-sidebar")
        ) {
          return;
        }
